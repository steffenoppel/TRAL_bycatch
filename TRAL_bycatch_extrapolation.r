...
#-------------------------------------------------
  ...
  for(scen in 1:n.scenarios){
    ...
    ## THE BREEDING POPULATION ##
    N.ad.surv.f[scen,tt] ~ dbin(fut.surv.change[tt,scen]*mean.phi.ad, max(1,round((Ntot.breed.f[scen,tt-1]-N.succ.breed.f[scen,tt-1])+N.atsea.f[scen,tt-1]-KILL[scen,tt-1,31])))           ### previous year's adults that survive
...
TRALbyc <- run.jags(data=jags.data, inits=inits, parameters, 
                    model="C:\\Users\\sop\\Documents\\Steffen\\RSPB\\TRAL_bycatch\\TRAL_bycatch_project.jags",
