##########################################################################
#
# TRISTAN ALBATROSS BYCATCH EXTRAPOLATION
#
##########################################################################
## written by steffen.oppel@rspb.org.uk
## code originally published in:
...
model {
    ...
    # - four projection scenarios based on proportional reduction of bycatch
    # -------------------------------------------------
...
#-------------------------------------------------
    ...
    for (gy in 1:2){    ## for good and poor monitoring years
      ...
      ## prop.fish.mort[gy] ~ dunif(0, 1)    # prior for the proportion of mortality resulting from fisheries, removed to calculate directly
    }
...
#-------------------------------------------------
    ...
    IM.kill[tt-1,1] <- max(1,round(JUV[tt-1])) - IM[tt,1,1]
    ...
    for(age in 2:3) {
      ...
      IM.kill[tt-1,age] <- max(1,round(IM[tt-1,age-1,3])) - IM[tt,age,1]
    }
    ...
    AD.kill[tt-1]<-round(Ntot.breed[tt-1]+N.atsea[tt-1]) - N.ad.surv[tt]
...
#-------------------------------------------------
    ...
    ## DERIVED PROPORTION OF MORTALITY THAT IS CAUSED BY BYCATCH
    ## bycatch data are for 2016
...
#-------------------------------------------------
  ...
  for(scen in 1:n.scenarios){
    ...
    for (tt in 2:FUT.YEAR){
      ...
      IM.f[scen,tt,1,1] ~ dbin(fut.surv.juv[scen], max(1,round(nestlings.f[scen,tt-1])))                                  ### number of 1-year old survivors 
      IM.f[scen,tt,1,2] <- 0
      IM.f[scen,tt,1,3] <- IM.f[scen,tt,1,1] - IM.f[scen,tt,1,2]
    
    for(age in 2:3) {
      IM.f[scen,tt,age,1]  ~ dbin(fut.surv.ad[scen], max(1,round(IM.f[scen,tt-1,age-1,3])))
...
parameters <- c("mean.phi.ad","mean.phi.juv","mean.fec",
                ...
                "mean.prop.fish.mort.ad","mean.prop.fish.mort.juv")
...
summary_tralipm <- summary(TRALbyc)
summary_tralipm_df <- as.data.frame(summary_tralipm)
addsummary_tralipm <- add.summary(TRALbyc,plots = runjags.getOption("predraw.plots"))
